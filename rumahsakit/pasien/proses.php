...
if(isset($_POST['add'])){
	...
	if(mysqli_num_rows($sql_cek_identitas) > 0){
		...
		mysqli_query($con, "INSERT INTO tb_pasien VALUES('$uuid', '$identitas', '$nama', '$tgl', '$jk', '$alamat', '$telp')") or die(mysqli_error($con));
	...
	if(mysqli_num_rows($sql_cek_identitas) > 0){
		...
		mysqli_query($con, "UPDATE tb_pasien SET nomor_identitas='$identitas', nama_pasien='$nama',tanggal_lahir='$tgl', jenis_kelamin='$jk', alamat='$alamat', no_telp='$telp' WHERE id_pasien='$id'") or die(mysqli_error($con));
		echo "<script>alert('Data berhasil ditambahkan');window.location='data.php';</script>";
	}
}
